...
def fetch_json(url: str) -> dict[str, object]:
    ...
    # urllib stubs type the context-manager response as Any; suppress here
    # then cast the read bytes so downstream json parse is typed.
    with urllib.request.urlopen(req, timeout=30) as response:  # pyright: ignore[reportAny]
        raw = cast("bytes", response.read())  # pyright: ignore[reportAny]
    return cast("dict[str, object]", json.loads(raw.decode("utf-8")))


def fetch_halfon(url: str) -> HalfonResponse:
    """Fetch and validate the halfon dataset in one call."""
    print(f"  Fetching {url}...")
    req = urllib.request.Request(url, headers={"User-Agent": "aoe2-llm-arena/1.0"})
    with urllib.request.urlopen(req, timeout=30) as response:  # pyright: ignore[reportAny]
        raw = cast("bytes", response.read())  # pyright: ignore[reportAny]
...
def _entity_to_record(entity_id: int, entity: HalfonEntity) -> dict[str, object]:
    """Build the common record fields (everything except is_building branch)."""
    return {
        "id": entity_id,
...
def process_halfon_data(payload: HalfonResponse) -> tuple[list[dict], list[dict]]:
    """Process the validated halfon payload into units and buildings lists."""
    units: list[dict] = []
    buildings: list[dict] = []
