...
The replay task uses `logger.exception(...)` on any exception (`arena/web/forks.py:248`). The trace shows up wherever `uvicorn` is writing logs. Most "stuck" cases turn out to be:
...
2. **Restart the server.** The lifespan teardown cancels all `fork_tasks` and `gather`s them with `return_exceptions=True` (`arena/web/server.py:241`), so a clean restart drains. Existing finalized runs in DuckDB are unaffected.
...
The replay path (`arena/web/forks.py:_replay`) has annotated lifecycle ordering — any new "stuck" failure mode is usually a violation of that ordering or a new SDK gotcha worth recording in the source as a comment.
