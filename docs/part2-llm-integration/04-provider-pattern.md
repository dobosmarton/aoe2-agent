...
`packages/gameplay-agent/src/providers/base.py:7-37` defines the contract:
...
Providers are registered in a simple dict at `packages/gameplay-agent/src/main.py:31-44`:
...
`packages/gameplay-agent/src/providers/claude.py:32-291` -- the only production provider.
...
1. Create `packages/gameplay-agent/src/providers/new_provider.py` implementing `BaseLLMProvider`
...
4. Register in `create_provider()` at `packages/gameplay-agent/src/main.py:33`
5. Add to `--choices` in the argparse definition at `packages/gameplay-agent/src/main.py:83`
