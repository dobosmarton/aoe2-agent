...
- **The primitives map cleanly.** `XADD` is publish, `XREAD BLOCK` is consumer wait, `MAXLEN ~ N` is the bounded-buffer policy, custom `<seq>-0` IDs preserve our `Seq` identity. The whole impl in `evaluation/redis_broker.py` is 458 lines including the cross-process `:open` sentinel and the SDK-bug workarounds.
