...
│        ▼                                                        │
│  ┌──────────────┐                                               │
│  │ YOLO26 Train │  Lambda Labs A100 (cloud)                     │
│  └──────────────┘                                               │
│        │                                                        │
│        ▼                                                        │
│  Trained Model (models/aoe2_yolo26.pt)                          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```bash
python generate_training_data.py --num-images 3000 --output detection/training_data
```
...
model = YOLO("detection/models/aoe2_yolo26.pt")
...
### Usage
```bash
python sld_extractor.py input.sld output.png --frame 0
```
...
scp -i ~/.ssh/your-key.pem ubuntu@<IP>:/home/ubuntu/runs/aoe2_yolo26/weights/best.pt ./detection/models/aoe2_yolo26.pt
...
class EntityDetector:
    def __init__(self, model_path="detection/models/aoe2_yolo26.pt"):
...
├── SLD_EXTRACTOR_README.md      # SLD format specification
