...
- **build**: Composite. REQUIRED: `building_key`. `x`/`y` are OPTIONAL — **omit them and the executor auto-places on open ground near the town centre** (it picks the emptiest spot and verifies the building landed). Only pass `x`/`y` for a specific *detected* empty tile. Executes: select idle villager → open the building's build menu → press its slot → place. Building keys are **menu-qualified**: a bare letter is the economic menu (q=House, w=Mill, e=Mining Camp, r=Lumber Camp, a=Farm, s=Blacksmith (Feudal+), t=Dock), two letters are `<menu><slot>` (`wq`=Barracks, `vd`=Market (Feudal+)). **ALWAYS use this instead of press(.)+press(q)+press(key)+click() separately** — it's 4x faster and it can't strand a placement on stale coordinates. A building whose menu isn't verified on this machine, or whose age hasn't been reached, is refused with an explanation — believe it and pick something else.
