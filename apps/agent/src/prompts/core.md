...
- **build**: Composite. REQUIRED: `menu` and `building_key`. The schema takes NO coordinates — the executor picks the spot after the camera settles, because selecting the villager moves the view and any point you computed would be stale. Menus: `menu="q"` economic (q=House, w=Mill, e=Mining Camp, r=Lumber Camp, a=Farm, s=Blacksmith, t=Dock), `menu="w"` military (q=Barracks, w=Archery Range, e=Stable), `menu="v"` advanced (d=Market). It places a Lumber Camp, Mining Camp or Mill next to its resource, and everything else on open ground near the town centre. **ALWAYS use this instead of a manual press+click sequence** — it's 4x faster and it verifies the placement landed.
- **research**: Composite. REQUIRED: `tech`. Goes to the right building and presses its key: castle_age, loom, wheelbarrow, horse_collar, double_bit_axe, gold_mining. The HUD spend confirms it next turn — if the cost never leaves your resources the button was greyed out and the failure detail says so. NEVER re-press a pending research.
