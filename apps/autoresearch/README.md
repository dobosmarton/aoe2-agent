...
Note: `memory_chain.py` was relocated to `gameplay-agent` since it's runtime-agent
code (loads cross-game memories into the executor's context, extracts new ones
after a game finishes). Autoresearch's orchestrator imports it from there.
...
uv run --package autoresearch python -m autoresearch.game_runner \
    --time-budget 600 --description "baseline"

# Full prompt loop (prompts you to start each game manually)
just autoresearch --max-experiments 5 --time-budget 1200

# Same loop, no baseline game
just autoresearch --no-baseline
```
